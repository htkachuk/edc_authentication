...
import re
from crypto import createPublicKeyECDSA, checkPublicKeyECDSA
...
def login():
    ...
    if algorythm == 'ECDSA':
        checkKey = checkPublicKeyECDSA(registered_user.password, username, password, number)

    # check the passwords
    if registered_user is None or checkKey == False:
